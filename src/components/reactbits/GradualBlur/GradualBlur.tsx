...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      let v: string | undefined = cfg[key];
...
const useIntersectionObserver = (ref: React.RefObject<HTMLDivElement>, shouldObserve: boolean = false) => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => { if (entry) setIsVisible(entry.isIntersecting); }, { threshold: 0.1 });
...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const config = useMemo(() => {
    const presetConfig = props.preset && PRESETS[props.preset] ? (PRESETS[props.preset] as Partial<GradualBlurProps>) : {};
