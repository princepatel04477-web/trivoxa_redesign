  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      if (enableMouseInteraction) {
        ...
        let cm0 = currentMouse[0] ?? 0.5;
        let cm1 = currentMouse[1] ?? 0.5;
        const tm0 = targetMouse[0] ?? 0.5;
        const tm1 = targetMouse[1] ?? 0.5;
        cm0 += smoothing * (tm0 - cm0);
        cm1 += smoothing * (tm1 - cm1);
        currentMouse[0] = cm0;
        currentMouse[1] = cm1;
        if (program.uniforms.uMouse?.value) {
          program.uniforms.uMouse.value[0] = cm0;
          program.uniforms.uMouse.value[1] = cm1;
        }
      } else if (program.uniforms.uMouse?.value) {
