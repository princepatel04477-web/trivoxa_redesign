...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    const uniforms = uniformsRef.current as Record<string, { value: unknown }> | null;
    if (!uniforms) return;

    const setU = (k: string, v: unknown) => {
      const entry = uniforms[k];
      if (entry) entry.value = v;
    };

    setU('uWispDensity', wispDensity);
    setU('uTiltScale', mouseTiltStrength);
    setU('uBeamXFrac', horizontalBeamOffset);
    setU('uBeamYFrac', verticalBeamOffset);
    setU('uFlowSpeed', flowSpeed);
    setU('uVLenFactor', verticalSizing);
    setU('uHLenFactor', horizontalSizing);
    setU('uFogIntensity', fogIntensity);
    setU('uFogScale', fogScale);
    setU('uWSpeed', wispSpeed);
    setU('uWIntensity', wispIntensity);
    setU('uFlowStrength', flowStrength);
    setU('uDecay', decay);
    setU('uFalloffStart', falloffStart);
    setU('uFogFallSpeed', fogFallSpeed);

    const { r, g, b } = hexToRGB(color || '#FFFFFF');
    const uColor = uniforms['uColor']?.value as { set: (r: number, g: number, b: number) => void } | undefined;
    uColor?.set(r, g, b);
