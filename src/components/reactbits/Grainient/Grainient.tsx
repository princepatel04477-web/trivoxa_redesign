...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => { if (entry) { isVisible = entry.isIntersecting; isVisible ? tryStart() : tryStop(); } },
      { threshold: 0 }
    );
  ...
  useEffect(() => {
    ...
    setVal('uTimeSpeed', timeSpeed);
    setVal('uColorBalance', colorBalance);
    setVal('uWarpStrength', warpStrength);
    setVal('uWarpFrequency', warpFrequency);
    setVal('uWarpSpeed', warpSpeed);
    setVal('uWarpAmplitude', warpAmplitude);
    setVal('uBlendAngle', blendAngle);
    setVal('uBlendSoftness', blendSoftness);
    setVal('uRotationAmount', rotationAmount);
    setVal('uNoiseScale', noiseScale);
    setVal('uGrainAmount', grainAmount);
    setVal('uGrainScale', grainScale);
    setVal('uGrainAnimated', grainAnimated ? 1.0 : 0.0);
    setVal('uContrast', contrast);
    setVal('uGamma', gamma);
    setVal('uSaturation', saturation);
    setVal('uCenterOffset', new Float32Array([centerX, centerY]));
    setVal('uZoom', zoom);
    setVal('uColor1', new Float32Array(hexToRgb(color1)));
    setVal('uColor2', new Float32Array(hexToRgb(color2)));
    setVal('uColor3', new Float32Array(hexToRgb(color3)));
    setVal('uLightMode', lightMode ? 1.0 : 0.0);
  }, [
