...
export default async function IndustryPage({ params }: { params: Promise<Params> }) {
  ...
  const overview = industry.shortDescription;
  ...
  return (
    <>
      ...
      <PageHero
        eyebrow={isService ? 'Industry · Service Exports' : onboarding ? 'Industry · onboarding' : 'Industry'}
        title={industry.name}
        lede={overview}
        trail={[
          { href: '/', label: 'Home' },
      ...
      <Section surface="light" tight className="pb-0 pt-md">
        <Container>
          <div className="max-w-5xl mx-auto">
            <SmartImage
              manifestId={`industry-${industry.slug}`}
      ...
      {/* A service industry with no physical catalogue rows is served by the Service Export
            ...
            <Reveal staggerChildren className="mt-2xl grid grid-cols-12 gap-md">
              {SERVICES.map((service) => (
                <Card key={service.slug} trace className="col-span-12 flex flex-col gap-sm p-lg md:col-span-6">
                  <h3 className="text-body-lg font-medium">{service.name}</h3>
        ...
        <Section surface="light" tight className="border-t surface-hairline">
          <Container>
            <SectionHeading
              ...
              title="Where these products go."
              lede="Key end uses, typical specifications, and destination markets for this sector."
            />
            <Reveal staggerChildren className="mt-xl grid grid-cols-12 gap-md">
              {categories.map((category) => {
                const commercial = CATEGORY_COMMERCIAL_CONTEXT[category.slug];
                return (
                  <Card key={category.slug} className="col-span-12 flex flex-col gap-sm p-lg md:col-span-6">
                    <h3 className="text-body-lg font-medium">{category.name}</h3>
                    {commercial ? (
                      <div className="flex flex-col gap-xs text-body-sm">
                        <p className="surface-muted">
                          <span className="surface-fg font-medium">Typical end use:</span>{' '}
                          {commercial.typicalEndUse}
                        </p>
                        <p className="surface-muted">
                          <span className="surface-fg font-medium">Order shape:</span>{' '}
                          {commercial.orderShape}
                        </p>
                        <p className="surface-muted">
                          <span className="surface-fg font-medium">What the buyer asks first:</span>{' '}
